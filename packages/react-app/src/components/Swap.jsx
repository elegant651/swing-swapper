  ...
  useEffect(() => {
    const getTokenList = async () => {
      ...
      try {
        ...
        const ethToken = WETH[ChainId.MAINNET];
  ...
  const getTrades = async () => {
    if (tokenIn && tokenOut && (amountIn || amountOut)) {
      ...
      getBestBridges({
        ...
        toSymbol: tokenOut,
        amount: tokenAmount,
        fromTokenAddress: tokens[tokenIn].address,
        fromChain,
        fromChainId: tokens[tokenIn].chainId,
        toChain,
        toChainId,
        // fromUserAddress,
        // toUserAddress,
    ...
    <Card
      title={
        <Space>
          ...
          <Typography>SwingSwapper</Typography>
        </Space>
      }
      extra={
        <Button
      ...
      <Space direction="vertical">
        ...
        <Row justify="center" align="middle">
          {bridgeRoutes.map(route => (
            <Card key={route.route[0].bridgeTokenAddress}>
              <div>destinationTxFee : {route.quote.destinationTxFee}</div>
              <div>bridgeFee : {route.quote.bridgeFee}</div>
              <div>duration : {route.duration}</div>
              <div>Bridge: {route.route[0].bridge}</div>
              <div>Bridge Token Address: {route.route[0].bridgeTokenAddress}</div>
            </Card>
          ))}
        </Row>
